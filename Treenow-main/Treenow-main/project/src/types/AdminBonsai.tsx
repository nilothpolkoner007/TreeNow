...
const AdminBonsai = () => {
  ...
  const fetchBonsai = async () => {
    ...
    try {
      const { data } = await axios.get(`${import.meta.env.VITE_BACKEND_SERVER}/api/bonsai`, {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data } = await axios.post(`${import.meta.env.VITE_BACKEND_SERVER}/api/bonsai`, formData, {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await axios.delete(`${import.meta.env.VITE_BACKEND_SERVER}/api/bonsai/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
