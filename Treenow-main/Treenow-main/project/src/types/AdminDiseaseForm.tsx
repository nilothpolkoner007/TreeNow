...
const AdminDiseaseForm: React.FC = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [treesRes, productsRes, diseasesRes] = await Promise.all([
          axios.get<Tree[]>(`${import.meta.env.VITE_BACKEND_SERVER}/api/trees`, config),
          axios.get<Product[]>(`${import.meta.env.VITE_BACKEND_SERVER}/api/products`, config),
          axios.get<Disease[]>(`${import.meta.env.VITE_BACKEND_SERVER}/api/diseases`, config),
        ]);
  ...
  const handleEdit = async (diseaseId: string) => {
    ...
    try {
      const { data } = await axios.get(`${import.meta.env.VITE_BACKEND_SERVER}/api/diseases/${diseaseId}`, {
  ...
  const handleDelete = async (diseaseId: string) => {
    ...
    try {
      await axios.delete(`${import.meta.env.VITE_BACKEND_SERVER}/api/diseases/${diseaseId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
    ...
    try {
      ...
      if (editId) {
        response = await axios.put(
          `${import.meta.env.VITE_BACKEND_SERVER}/api/diseases/${editId}`,
          {
            name,
            tree: treeId,
            symptoms,
        ...
        response = await axios.post(
          `${import.meta.env.VITE_BACKEND_SERVER}/api/diseases`,
          {
            name,
            tree: treeId,
            symptoms,
