...
const AdminTree = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editId) {
        // Update tree
        const response = await axios.put(`${import.meta.env.VITE_BACKEND_SERVER}/api/trees/${editId}`, formData, {
        ...
        // Create new tree
        const response = await axios.post(`${import.meta.env.VITE_BACKEND_SERVER}/api/trees`, formData, {
          headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
        });
        data = response.data;
  ...
  const handleEdit = async (id: string) => {
    ...
    try {
      const { data } = await axios.get(`${import.meta.env.VITE_BACKEND_SERVER}/api/trees/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await axios.delete(`${import.meta.env.VITE_BACKEND_SERVER}/api/trees/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
